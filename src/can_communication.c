...
void prompt(int root_rank, MPI_Comm comm, int nb_proc) {
    ...
    while (1) {
        ...
        if (strncmp(str_insert, user_cmd, 6) == 0) {
            printf("---insert : %s\n", user_cmd);
            char number[MAX_LEN];
        ...
        else if (strcmp(str_all, user_cmd) == 0) {
            printf("---insert all: %s\n", user_cmd);
            CAN_Root_Process_Job(root_rank, comm, 0, nb_proc, nb_proc);
        }
...
int CAN_Root_Process_Job(int root_rank, MPI_Comm comm, int proc_min, int proc_max, int nb_proc) {
    ...
    for(int i = proc_min; i < proc_max; i++) {
        if(i != root_rank) {
            ...
            if(main_loop_tag == ACK_TAG_BOOTSTRAP) {
                ...
                MPI_Recv(&main_loop_buffer_int[0] ,1 , MPI_INT, i, ACK, comm, &main_loop_status); // le noeud i a été inséré
